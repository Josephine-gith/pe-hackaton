...
social = df["Social support"][
    (df["Social support"] > 0.4) & (df["Healthy life expectancy at birth"] > 35)
]
healthy = df["Healthy life expectancy at birth"][
    (df["Social support"] > 0.4) & (df["Healthy life expectancy at birth"] > 35)
]
plt.plot(social, healthy, "+")
regression = scipy.stats.linregress(social, healthy)
plt.plot(
    np.linspace(0.4, 1, 1000), regression[0] * np.linspace(0.4, 1, 1000) + regression[1]
)
